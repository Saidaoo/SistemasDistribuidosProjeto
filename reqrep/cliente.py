...
class BBSClient:
    ...
    def send_request(self, service, data):
        """Envia requisição ao servidor - ✅ MODIFICADO para MessagePack"""
        request = {
            "service": service,
            "data": data
        }
        
        # ✅ MODIFICADO: Usa MessagePack em vez de JSON
    ...
    def login(self, username):
        ...
        if response["data"]["status"] == "sucesso":
            ...
            print(f"❌ Erro no login: {response['data'].get('description', 'Erro desconhecido')}")
            return False
    ...
    def create_channel(self, channel_name):
        ...
        if response["data"]["status"] == "sucesso":
            ...
            print(f"❌ Erro ao criar canal: {response['data'].get('description', 'Erro desconhecido')}")
            return False
    ...
    def publish_message(self, channel, message):
        ...
        if response["data"]["status"] == "OK":
            ...
            print(f"❌ Erro: {response['data'].get('message', 'Erro desconhecido')}")
            return False
    ...
    def listen_messages_background(self):
        """Escuta mensagens em background (thread) - ✅ CORRIGIDO para MessagePack"""
        while self.listening:
            try:
                # ✅ CORRIGIDO: Recebe multipart com MessagePack
                message_parts = self.sub_socket.recv_multipart(zmq.NOBLOCK)
                
                # ✅ CORRIGIDO: Verifica se tem pelo menos 2 partes (tópico + dados)
                if len(message_parts) >= 2:
                    topic_bytes = message_parts[0]
                    msg_bytes = message_parts[1]
                    
                    try:
                        # ✅ Tenta decodificar o tópico como UTF-8
                        topic = topic_bytes.decode('utf-8')
                        
                        # ✅ Desempacota com MessagePack
                        msg_data = msgpack.unpackb(msg_bytes, raw=False)
                        ...
                        print(f"\n⚠️  Erro: Tópico com encoding inválido")
                        print(f"BBS {self.current_user} > ", end="", flush=True)
                    except Exception as e:
                        print(f"\n⚠️  Erro ao processar mensagem: {e}")
                        print(f"BBS {self.current_user} > ", end="", flush=True)
                
                # ✅ CORRIGIDO: Mensagens com apenas 1 parte podem ser do formato antigo
                elif len(message_parts) == 1:
                    try:
                        # Tenta processar como mensagem antiga (JSON) por compatibilidade
                        old_message = message_parts[0].decode('utf-8')
                        if ' ' in old_message:
                            topic, data = old_message.split(' ', 1)
                            msg_data = json.loads(data)
                            
                            if msg_data.get("type") == "direct_message":
                                print(f"\n💌 [MENSAGEM PRIVADA - LEGADO] De {msg_data['from']}: {msg_data['message']}")
                                print(f"BBS {self.current_user} > ", end="", flush=True)
                            elif msg_data.get("type") == "channel_message":
                                if msg_data['user'] != self.current_user:
                                    print(f"\n📢 [#{msg_data['channel']} - LEGADO] {msg_data['user']}: {msg_data['message']}")
                                    print(f"BBS {self.current_user} > ", end="", flush=True)
                    except:
                        # Ignora mensagens incompatíveis
                        pass
